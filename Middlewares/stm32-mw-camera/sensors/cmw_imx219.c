...
static int32_t CMW_IMX219_getMirrorFlipConfig(uint32_t Config)
{
  ...
  switch (Config)
  {
  case CMW_MIRRORFLIP_NONE: ret = IMX219_MIRROR_FLIP_NONE; break;
  case CMW_MIRRORFLIP_FLIP: ret = IMX219_FLIP; break;
  case CMW_MIRRORFLIP_MIRROR: ret = IMX219_MIRROR; break;
  case CMW_MIRRORFLIP_FLIP_MIRROR:
  default: ret = IMX219_MIRROR_FLIP; break;
  }
...
static int32_t CMW_IMX219_SetFramerate(void* io_ctx, int32_t framerate)
{
  ...
  for (int i = 0; i < ARRAY_SIZE(available_imx219_fps); i++)
    if (framerate == available_imx219_fps[i]) return IMX219_SetFramerate(&((CMW_IMX219_t*)io_ctx)->ctx_driver, framerate);
...
{
  /* Note: According to the IMX219 datasheet (E18112A84), the power-on sequence
   ...
   * be followed.
   *
   * MB1938 N6-DK and MB1854 IMX219 board IO mapping:
...
static void CMW_IMX219_VsyncEventCallback(void* io_ctx, uint32_t pipe)
{
  ...
  switch (pipe)
  {
  case DCMIPP_PIPE0: ISP_IncDumpFrameId(&((CMW_IMX219_t*)io_ctx)->hIsp); break;
  case DCMIPP_PIPE1:
    ISP_IncMainFrameId(&((CMW_IMX219_t*)io_ctx)->hIsp);
    ISP_GatherStatistics(&((CMW_IMX219_t*)io_ctx)->hIsp);
    break;
  case DCMIPP_PIPE2: ISP_IncAncillaryFrameId(&((CMW_IMX219_t*)io_ctx)->hIsp); break;
  }
#endif
}
